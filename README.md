...
## 🤔 회고
주제가 이미지 분류여서 데이터 전처리와 EDA를 하는 데 한계가 있어서 다소 아쉬웠음 </br>
데이터 증강과정을 거쳤는데 성능이 감소하여 원인을 찾아봄 </br>
-> 병변 데이터는 Random증강을 조심! (병변이 사라질 수도 있음) </br>
-> 증강 시, 같은 클래스의 서로 다른 데이터의 부분을 교차하는 이미지 증강을 해보고 싶어서 실제로 이미지를 교차하였으나 코드오류로 실행하지 못하였음 </br>
